...
const deleteComissionValueById = `-- name: DeleteComissionValueById :exec
DELETE FROM comission_values WHERE id = $1
`

func (q *Queries) DeleteComissionValueById(ctx context.Context, id uuid.UUID) error {
	_, err := q.exec(ctx, q.deleteComissionValueByIdStmt, deleteComissionValueById, id)
	return err
}

const deleteFlowBalanceById = `-- name: DeleteFlowBalanceById :exec
DELETE FROM cash_flow_balance WHERE id = $1
`

func (q *Queries) DeleteFlowBalanceById(ctx context.Context, id uuid.UUID) error {
	_, err := q.exec(ctx, q.deleteFlowBalanceByIdStmt, deleteFlowBalanceById, id)
	return err
}
...
const updateComissionValue = `-- name: UpdateComissionValue :one
UPDATE comission_values
SET 
  percentage = COALESCE($2, percentage),
  who_updated_id = COALESCE($3, who_updated_id),
  type = COALESCE($4, type)
WHERE id = $1
RETURNING id, percentage, company_id, who_created_id, who_updated_id, type
`

type UpdateComissionValueParams struct {
	ID           uuid.UUID `json:"id"`
	Percentage   *float64  `json:"percentage"`
	WhoUpdatedID *string   `json:"who_updated_id"`
	Type         *string   `json:"type"`
...
func (q *Queries) UpdateComissionValue(ctx context.Context, arg UpdateComissionValueParams) (ComissionValues, error) {
	row := q.queryRow(ctx, q.updateComissionValueStmt, updateComissionValue,
		arg.ID,
		arg.Percentage,
		arg.WhoUpdatedID,
		arg.Type,
